...
The symbol table below corrects a four-slot rotation in KFPS's own
`editor.js` symbol table against the actual game meshes. This was verified
three independent ways: a static source diff, `Resources/Vinyls` mesh
preview dumps, and live in-game runtime (reproducing the "Ninja€s" bug
exactly). The correction also surfaces 14 previously-unused Upper-page
slots per font (digits 1-0 and `! ? @ &`) with preview assets already
present in every font. Two open items remain, neither of which affects
correctness of what's implemented here:
- `;`/`:` at 38/39 (not the reverse) is the mesh-based reading; confirming
  the *input* mapping agrees with the mesh appearance still needs a single
  in-game keystroke test. Worst case these two swap.
