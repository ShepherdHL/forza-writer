...
- **Forza Font Text**: type free-flowing, proportionally-spaced text using
  one of FH6's 11 native in-game vinyl fonts
  (`forza_writer.layout.layout_forza_text`), not a font this tool generated
  or one installed on the machine. Every glyph is FH6's own native letter
  mesh, placed and scaled (not approximated or traced), so output is real,
  final, ready-to-use shapes rather than a fitted design. Sibling to ASCII
  Art below, which lays the same native fonts out on a fixed monospace grid
  instead of free-flowing text. **Preview** renders the layout; **Save
  .json…** and **Save .fabric-project.json…** export it for use elsewhere or
  in the KFPS Fabric Editor.
- **Generator**: pick a font and characters, choose an output mode (Custom
  ...
  **Browse on machine…** picks one font file directly, bypassing the
  installed-font list entirely (same split as bvzrays' forza-painter-fh6's own
...
- **ASCII Art**: paste existing ASCII art and place it, character-for-
  character, using one of the same 11 native in-game vinyl fonts as Forza
  Font Text (`forza_writer.ascii_grid`), on a strict, uniform grid: every
  cell advances the same fixed width/height regardless of its glyph, so
  columns stay aligned the way they were pasted. Unsupported characters can
  be remapped before generating. **Preview** renders the grid; **Save
  .json…** exports it.
- **Glyph Inspector**: browse every glyph a font supports, categorized by
  Unicode block, and inspect one at large scale against the font's own
  metrics. Three views per glyph: **Reference** (the glyph rendered straight
  from the font's outline, with ascender/cap-height/x-height/baseline/
  descender guides), **Generated** (runs the real generation pipeline, the
  same fitter Direct Generator uses, cached per font/character/compute
  backend) and **Compare** (diffs Generated against either the font's own
  outline or a hand-loaded single-glyph shape file, the same single-glyph
  `{"shapes": [...]}` format Configurator's manual overrides use).
...
- **Composer**: type arbitrary text using an already-generated fontpack's
  ...
  without touching glyph generation.
- **Layer Effects**: build a Layered Glyph Effect from one source glyph,
  with inset, outset, translate, scale, rotate and boolean-combination
  layers, each independently colored and ordered, run through the same
  primitive generator every other tab uses (`forza_writer.layered_effects`).
  Layers can be added, duplicated, reordered or deleted; a full stack can be
  saved as a named preset and reloaded later. Preview renders the effect
  against sample text.
- **Settings**: the reference `.modelbin` path and separate output
  directories for each generation workflow (persisted to
  `%LOCALAPPDATA%\forza-writer\settings.json`), plus generation processor
  selection. **Auto** prefers NVIDIA CUDA, falling back to CPU when CUDA/CuPy
  is unavailable; explicit CUDA and CPU choices are also available. AMD/Intel
...
is a hole cut from it. This is wrong for decorative/display fonts with
disjoint components (separate accent marks, dots, swash flourishes), which
can list a small decorative contour before the main letterform and get it
triangulated backwards. Both now determine fill-vs-hole by actual geometric
nesting instead of contour order (`gen_modelbin.group_contours_by_nesting`,
one earcut() call per disjoint island; `primitive_fit.rasterize_contours`,
even-odd XOR fill).
...
candidate scoring runs on NVIDIA CUDA when the optional `cupy-cuda12x`
package (see `requirements.txt`) is installed, while Pillow outline/template
preparation and final local refinement remain on the CPU. Use
`--compute-backend cuda` to require CUDA
...
bvzrays' `forza-painter-fh6` and shipped in v1.9.5. It rasterizes the glyph
...
- **Rectilinear glyphs** (every contour edge horizontal or vertical, as in
  blocky stencil fonts like Amarillo USAF) go to an exact rectangle
  decomposition (`forza_writer/rect_decompose.py`). The answer is computed,
  not searched for: the cover is exact (IoU 1.0), it's effectively instant
  and each rectangle becomes one Square scaled independently on X and Y.
...
(`data[6]=1`, `"mask": true`, which punches transparency through whatever's
below) cut into its negative space, revealing the letterform. Both are exact
...
it's a real win on real fonts too. The Standard Galactic Alphabet's `X`
needs 14 shapes filled directly vs. 10 as a stencil, `Z` needs 12 vs. 7.
...
everywhere. This trades away stencil's shape-count savings on glyphs where
it would have won, in exchange for using only the long-verified plain-fill
path with no mask layers at all.
...
pick one (its own font field, decoupled from Generator's, with a "Use
current from Generator" button that copies Generator's selection over on
demand) and it lists every glyph the font has, via
`forza_writer/charset.py::charset_from_font`. Reviewing/overriding the
font's whole glyph set is the point, so unlike
...
- **Auto**: the default, letting `primitive_fit.fit_placements` decide (see
  "Stencil mode" above).
...
`"mask"`, so no glyph regeneration is needed:
